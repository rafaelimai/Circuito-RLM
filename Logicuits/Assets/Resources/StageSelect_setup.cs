...
public class StageSelect_setup : MonoBehaviour {
	...
	void OnGUI () {
		...
		for (int i = 1; i <= 8; i++) {
			if (GUI.Button(new Rect ((Screen.width - BUTTON_WIDTH)*1f/3f, Screen.height * (7+i)/16, BUTTON_WIDTH, guiSkin.button.fontSize), "Level "+System.Convert.ToString(i))) {
				Level_setup.currentLevel = i;
				Application.LoadLevel("leveleditor");
			}
		}
		for (int i = 9; i <= 16; i++) {
			if (GUI.Button(new Rect ((Screen.width - BUTTON_WIDTH)*2f/3f, Screen.height * (i-1)/16, BUTTON_WIDTH, guiSkin.button.fontSize), "Level "+System.Convert.ToString(i))) {
				Level_setup.currentLevel = i;
				Application.LoadLevel("leveleditor");
			}
		}
